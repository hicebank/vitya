...
from vitya.exceptions import (
    ValidationError,
    InnValidationError,
    KppValidationError,
    BicValidationError,
    OgrnValidationError,
    SnilsValidationError
)
...
def validate_inn(inn: str) -> None:
    ...
    """
    if not inn:
        raise InnValidationError('inn is empty')

    if not isinstance(inn, str):
        raise InnValidationError('inn should be passed as string')

    if not re.fullmatch(r'[0-9]+', inn):
        raise InnValidationError('inn can contain only numbers')
    ...
    if len(inn) == 10:
        ...
        if n10 != int(inn[9]):
            raise InnValidationError(f'wrong checksum on last digit: {inn[9]}; expected: {n10}')
        return

    if len(inn) == 12:
        n11 = count_checksum(inn, [7, 2, 4, 10, 3, 5, 9, 4, 6, 8])
        if n11 != int(inn[10]):
            raise InnValidationError(f'wrong checksum on pre-last digit: {inn[10]}; expected: {n11}')

        n12 = count_checksum(inn, [3, 7, 2, 4, 10, 3, 5, 9, 4, 6, 8])
        if n12 != int(inn[11]):
            raise InnValidationError(f'wrong checksum on last digit: {inn[11]}; expected: {n12}')
        return

    raise InnValidationError('wrong size of inn, it can be 10 or 12 chars only')
...
def validate_kpp(kpp: str) -> None:
    ...
    """
    if not kpp:
        raise KppValidationError('kpp is empty')

    if not isinstance(kpp, str):
        raise KppValidationError('kpp should be passed as string')

    if len(kpp) != 9:
        raise KppValidationError('wrong size of kpp, it can be 9 chars only')

    if not re.fullmatch(r'[0-9]{4}[0-9A-Z]{2}[0-9]{3}', kpp):
        raise KppValidationError('wrong kpp')
...
def validate_bic(bic: str) -> None:
    ...
    if not isinstance(bic, str):
        raise BicValidationError('bic should be passed as string')

    if len(bic) != 9:
        raise BicValidationError('wrong size of bic, it can be 9 chars only')

    if not re.fullmatch(r'04[0-9]+', bic):
        raise BicValidationError('wrong bic')
...
def validate_ogrn(ogrn: str) -> None:
    ...
    """
    if not ogrn:
        raise OgrnValidationError('ogrn is empty')

    if not isinstance(ogrn, str):
        raise OgrnValidationError('ogrn should be passed as string')

    if len(ogrn) != 13 and len(ogrn) != 15:
        raise OgrnValidationError('wrong size of ogrn, it can be 13 chars only')

    if not re.fullmatch(r'[1-9][0-9]+', ogrn):
        raise OgrnValidationError('wrong ogrn')

    if len(ogrn) == 13:
        n13 = int(ogrn[:-1]) % 11 % 10
        if n13 != int(ogrn[12]):
            raise OgrnValidationError(f'wrong checksum on pre-last digit: {ogrn[12]}; expected: {n13}')

    if len(ogrn) == 15:
        n15 = int(ogrn[:-1]) % 13 % 10
        if n15 != int(ogrn[14]):
            raise OgrnValidationError(f'wrong checksum on pre-last digit: {ogrn[14]}; expected: {n15}')
...
def validate_snils(snils: str) -> None:
    ...
    """
    if not snils:
        raise SnilsValidationError('snils is empty')

    if not isinstance(snils, str):
        raise SnilsValidationError('snils should be passed as string')

    if not re.fullmatch(r'[0-9]{3}-[0-9]{3}-[0-9]{3} [0-9]{2}', snils):
        raise SnilsValidationError('wrong snils')
    ...
    if checksum_str != snils[-2:]:
        raise SnilsValidationError(f'wrong checksum: {snils[-2:]}; expected: {checksum_str}')
